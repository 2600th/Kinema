...
**Final variance correction:** On 2026-07-22, the sequential three-window measurement was replaced by four fresh matched pairs in balanced `AB/BA/BA/AB` order. The stronger gate reproduced a 1.133 median at 0.75 scale. Reducing only the compatibility-post scale to 0.70 and skipping vignette shader work when its balanced-profile uniform is zero retained half-float grading, LUT, and 2x MSAA. Two isolated runs measured median pairwise ratios of 1.0668 and 1.0664; subsequent whole-suite runs measured 1.0716 and 1.0718, all inside the unchanged 1.10 limit with zero runtime errors.
...
2. Create `EffectComposer` with `RenderPass` and one reusable raw output-grade pass for tone mapping, output conversion, LUT, and vignette; apply capped 2x samples to both internal render targets and the measured 0.70 compatibility scale.
